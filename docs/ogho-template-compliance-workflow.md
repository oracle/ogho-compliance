...
Repositories should reference the centrally maintained action rather than copying the validator. This applies both to repositories created from `oracle/template-repo` and to existing repositories. Create `.github/workflows/ogho-template-compliance.yml` with the following content:
